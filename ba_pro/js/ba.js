...
(function(BA, $, undefined) {
    ...
    BA.lastSignals = { 'BA_SIGNAL_1': undefined, 'BA_SIGNAL_2': undefined };
    ...
    BA.prepareOneSignal = function(signal_name) {
        ...
        step = ((endFreq - startFreq) / steps).toFixed(2);
        ...
        if ($('#BA_AUTOSCALE_BTN').hasClass('active')) {
            ...
            var l = one_signal['value'].length;
            var v = parseFloat(one_signal['value'][l - 1]).toFixed(3)*1;
            var v2 = 0;
            var yaxis;
            ...
            if (v >= 0)
            {
                ...
                if (l == 1) {
                    $('#' + signal_name + '_MAX').val(((1+v) * 1.2).toFixed(1));
                    yaxis.options.max  = (1+v) * 1.2;
                } else if(v > v2) {
                    $('#' + signal_name + '_MAX').val((v * 1.2).toFixed(1));
            ...
            if (v >= 0)
            {
                ...
                if (l == 1) {
                    $('#' + signal_name + '_MIN').val((1.2/(-1+v)).toFixed(1));
                    yaxis.options.min  = 1.2/(-1+v);
                }
                else if(v < v2) {
                    $('#' + signal_name + '_MIN').val((1.2/v).toFixed(1));
    ...
    BA.initPlot = function(update) {
        ...
        BA.graphCache.plot = $.plot(BA.graphCache.elem, [{data: lastsig1, color: '#f3ec1a', label: "Amplitude"}, {data: lastsig2, color: '#31b44b', label: "Phase", yaxis: 2}], options);
    ...
    BA.drawSignals = function() {
        ...
        if (BA.running == true) {
            ...
            BA.graphCache.plot.setData([{data: lastsig1, color: '#f3ec1a', label: "Amplitude"}, {data: lastsig2, color: '#31b44b', label: "Phase", yaxis: 2}]);
